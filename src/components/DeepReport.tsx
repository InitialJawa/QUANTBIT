    ...
    <div id="deep-report-container" className="space-y-5">
      ...
      {!report && !isGenerating && (
        <motion.div 
          ...
          className="bg-[#050505] rounded-2xl border border-white/[0.04] p-6 text-center"
        >
          <div className="w-12 h-12 bg-white/5 text-[#00c9a5] rounded-xl flex items-center justify-center mx-auto mb-4 border border-white/[0.06]">
            <TickerLogo ticker={stock.ticker.replace(".JK", "")} size="lg" fallbackColor={stock.logoColor} />
          </div>
          <h3 className="text-body font-bold text-white mb-2">Belum Ada Analisis</h3>
          <p className="text-caption text-white/50 mb-6">
            Analisis kondisi lengkap dari {stock.ticker} menggunakan kecerdasan buatan Gemini.
          </p>

          <div className="space-y-3 mb-6 text-left">
            <label className="block text-label font-bold text-white/40 uppercase tracking-widest">
            ...
            <input
              ...
              className="w-full text-caption px-4 py-3 rounded-xl border border-white/[0.06] outline-none focus:ring-1 focus:ring-[#00c9a5] transition-all bg-[#0a0a0a] text-white placeholder:text-white/30"
            />
          </div>

          <button
            onClick={handleGenerateClick}
            className="inline-flex items-center gap-2 bg-[#00c9a5] hover:bg-[#00a880] text-black font-bold px-6 py-3 rounded-xl text-caption uppercase tracking-wider transition-all cursor-pointer"
          >
      ...
      {/* Generating Loader */}
      {isGenerating && (
        <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-12 text-center flex flex-col items-center justify-center space-y-4">
          <div className="relative">
            <div className="w-12 h-12 border-4 border-white/[0.06] border-t-[#00c9a5] rounded-full animate-spin"></div>
            <Sparkles className="w-6 h-6 text-[#00c9a5] absolute top-3 left-3 animate-pulse" />
          </div>
          <div>
            <h4 className="font-bold text-white text-caption">Memproses Data Finansial</h4>
            <p className="text-label text-white/40 mt-1 animate-pulse">
      ...
      {report && !isGenerating && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Recommendation Card */}
            <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-5 flex flex-col justify-between">
              <div>
                <span className="text-label uppercase font-bold text-white/40 tracking-widest">
                  Analisis Skenario
                </span>
                <h4 className="text-caption font-semibold text-white/80 mt-1">Sikap Pasar</h4>
              </div>
              <div className="my-4">
              ...
              <div className="text-label text-white/40 flex items-center gap-1.5 pt-3 border-t border-white/[0.04]">
                <ShieldCheck className="w-3.5 h-3.5 text-[#00c9a5]" />
            ...
            <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-5 flex flex-col justify-between">
              <div>
                ...
                <h4 className="text-caption font-semibold text-white/80 mt-1">Harga Wajar Saham</h4>
              </div>
              <div className="my-2">
                <div className="flex justify-between items-baseline">
                  <span className="text-label text-white/40">Harga Wajar:</span>
                  <span className="text-body font-bold text-[#00c9a5] font-mono">
                    Rp {report.fairValue.estimatedValue?.toLocaleString("id-ID") || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between items-baseline mt-1.5">
                  <span className="text-label text-white/40">Harga Saat Ini:</span>
                  <span className="text-label font-semibold text-white/80 font-mono">
                    Rp {stock.currentPrice.toLocaleString("id-ID")}
                  </span>
                </div>
              </div>
              <div className="pt-2 border-t border-white/[0.04] flex items-center justify-between">
                <span className="text-label text-white/40 font-medium uppercase tracking-wider">
                  Status Valuasi:
                </span>
            ...
            <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-5 flex flex-col justify-between">
              <div>
                ...
                <h4 className="text-caption font-semibold text-white/80 mt-1">Batas Aman Harga</h4>
              </div>
              <div className="my-2">
                {(() => {
              ...
              <div className="text-label text-white/40 flex items-center gap-1.5 pt-2 border-t border-white/[0.04]">
                <Target className="w-3.5 h-3.5 text-[#00c9a5]" />
          ...
          <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-5">
            <h4 className="text-label uppercase font-bold text-white/40 tracking-widest mb-2.5">
              Rangkuman Laporan Singkat
            </h4>
            <div className="text-caption leading-relaxed text-white/70 whitespace-pre-line">
              {report.summary}
            </div>
          </div>
          ...
          {/* SWOT Grid */}
          <div>
            <h4 className="text-caption font-bold text-white/70 uppercase tracking-widest mb-3 flex items-center gap-2">
              <Sparkles className="w-3.5 h-3.5 text-[#00c9a5]" />
          ...
          <div className="bg-[#050505] rounded-2xl border border-white/[0.04] p-5 overflow-hidden">
            <h4 className="text-label uppercase font-bold text-white/40 tracking-widest mb-4">
                ...
                <tbody className="divide-y divide-white/[0.03] text-label">
                  {report.keyRatios?.map((ratio, index) => (
                    <tr key={index} className="hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 px-1 font-medium text-white/90">{ratio.label}</td>
                      <td className="py-3 px-4 font-mono font-bold text-[#00c9a5]">{ratio.value}</td>
          ...
          {/* Growth Outlook box */}
          <div className="bg-[#050505] border border-white/[0.04] rounded-2xl p-5 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-2">
              <span className="inline-flex items-center gap-1.5 bg-[#00c9a5]/10 border border-[#00c9a5]/20 text-[#00c9a5] text-label px-2 py-0.5 rounded-full font-bold uppercase tracking-widest">
                <Target className="w-3 h-3" />
                Pandangan AI Kedepan
              </span>
              <h4 className="text-body font-bold text-white pt-1">Pertumbuhan & Potensi Saham</h4>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-xl border border-[#00c9a5]/20 bg-[#00c9a5]/5">
            <p className="text-label text-white/50">
              Analisis di-generate AI pada: <strong className="text-white/70">{new Date(report.timestamp).toLocaleString("id-ID")}</strong>.
            </p>
            <button
              onClick={() => onGenerateReport(customFocus || undefined)}
              className="px-4 py-2 bg-[#00c9a5] hover:bg-[#00a880] text-black rounded-lg text-label font-bold flex items-center gap-1.5 cursor-pointer uppercase tracking-wider transition-colors"
            >
