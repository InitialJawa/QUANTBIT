...
import { enrichDayDataWithScores } from "./scoring";

export function runStrategy(input: StrategiesInput): BacktestResult {
  const { dayData: rawInput, config, profileWeights: inputWeights, universeTickers, fees = DEFAULT_FEES, scoreLookup } = input;
  let currentWeights = { ...inputWeights };

  const getScoresForDate = (date: string): Record<string, { quality: number; growth: number; value: number; momentum: number; dividend: number }> | undefined => {
  ...
  // M10 fix: compute scores from price data when stockNormScores is missing
  // or when scores are all equal (degenerate). This replaces the old dependency
  // on stock_scores D1 table which only had momentum data (Q/G/V/D = null → 50).
  const hasMeaningfulScores = rawInput.some(d => {
    const ns = d.stockNormScores;
    if (!ns) return false;
    const tickers = Object.keys(ns);
    if (tickers.length === 0) return false;
    const s = ns[tickers[0]];
    return s.quality !== 50 || s.growth !== 50 || s.value !== 50;
  });
  if (!hasMeaningfulScores) {
    enrichDayDataWithScores(rawInput);
  }
  ...
  const dayData: BacktestDayData[] = rawInput.map(d => {
    const normScores = d.stockNormScores || getScoresForDate(d.date);
    if (normScores && typeof normScores === "object") {
      ...
      const tickersInDay = Object.keys(normScores);
